...
void effects::Tempo::reset() noexcept
{
    bpm = 120.0;
}

void effects::Tempo::update(const juce::AudioPlayHead* playhead) noexcept
{
    reset();
...
double effects::Tempo::getMillisecondsForNoteLength(int index) const noexcept
{
    return 60000.0 * noteLengthMultipliers[size_t(index)] / bpm;
}

void effects::DelayFX::prepare(juce::dsp::ProcessSpec& spec)
{
...
void effects::DelayFX::process(juce::dsp::ProcessContextReplacing<float>& context)
{
    if (bypass) {
        return;
    }
